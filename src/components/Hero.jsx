...
const Hero = () => {
  ...
  useEffect(() => {
    ...
    }, 1600); // Change to your desired interval (1 second for typing effect, plus the duration of animation)
          ...
          >
            <h1 className='font-bold sm:text-7xl text-4xl'>
              Full Stack {developer.map((char, index) => (
                <motion.span className='text-blue-800 type-effect' key={index} variants={letterAnimation} transition={{ duration: 0.2 }}> 
                  {char}
                </motion.span>
              ))}
            </h1>
